/*
 ...
 Version 1.0.2 is for use with the prototype PMLDS system that uses the red Sparkfun protoboard for the
 circuit and is the origin NRL-PMLDS system. Its pin assignments are slightly different than the PCB-based
 PMLDS systems. This version adds a 1 minute pump prime at 2.5V for the EPC to generate flow but not
 regulate the flow until after the first minute. There is a problem with starting the system where the
 controller will open the EPC to full scale and the flow meter will read 0 uL/min because off scale
 on the flow meter is 0 uL/min; thus, the controller never throttles back the EPC pressure to regulate
 the flow even though liquid is still be pumped through the system. The 1 minute priming stage allows
 for bubbles and air to purged from the system and generate a stable liquid flow. Once a stable
 liquid flow is established, the microcontroller can begin PID control to the target flow rate.
 
 Version 1.0.2 also fixes an issue with the controller displaying flow rate even when the flow meter
 is not connected.
 */
...
const int DEFAULT_FLOW_RATE = 400; // uL/min * 10
...
{ 
  ...
  // Set the output voltage to 0 PSI, 0V until connection has been established.
  writeDAC(dacNumber);

  // Print some startup information to the user.
  lcd.print("NRL-CANS v1.0.2");
  lcd.setCursor(0, 1);
